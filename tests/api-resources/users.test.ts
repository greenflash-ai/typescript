...
describe('resource users', () => {
  ...
  test('getUserAnalytics: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.users.getUserAnalytics('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
    mode: 'simple',
  ...
  test('getUserSegments', async () => {
    const responsePromise = client.users.getUserSegments('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
