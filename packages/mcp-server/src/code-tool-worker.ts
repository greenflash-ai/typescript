...
import { Greenflash, ClientOptions } from 'greenflash';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Greenflash)`
    : `const run: (${functionSource.client}: Greenflash) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
