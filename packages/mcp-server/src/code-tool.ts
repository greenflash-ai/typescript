...
import { ClientOptions } from 'greenflash';

const prompt = `Runs JavaScript code to interact with the Greenflash API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      client_envs: JSON.stringify({
        GREENFLASH_API_KEY: readEnv('GREENFLASH_API_KEY') ?? client.apiKey ?? undefined,
        GREENFLASH_BASE_URL: readEnv('GREENFLASH_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'greenflash-public-api',
      code,
      intent,
      client_opts: {},
